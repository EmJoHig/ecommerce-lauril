...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params; const [product, settings] = await Promise.all([getCatalogService().getProduct(slug), getPublicStoreSettings()]); if (!product) return {};
  const description = product.seoDescription ?? product.shortDescription ?? product.description ?? `Conocé ${product.name} en ${settings.storeName}.`;
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params; const [product, settings] = await Promise.all([getCatalogService().getProduct(slug), getPublicStoreSettings()]); if (!product) notFound();
  const canonical = `${getServerEnv().APP_URL}/producto/${product.slug}`;
  ...
  return <section className="product-detail section"><script dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, "\\u003c") }} type="application/ld+json" /><div className="product-detail__heading"><Link className="text-link" href="/productos">← Volver al catálogo</Link><p className="eyebrow">{product.categories.map((category) => category.name).join(" · ") || settings.storeName}</p><h1>{product.name}</h1><p>{product.description ?? product.shortDescription}</p></div><ProductDetail images={product.images} name={product.name} variants={product.variants.map((variant) => ({ id: variant.id, sku: variant.sku, name: variant.name, price: formatMoney(variant.currentPriceInCents), regularPrice: variant.promotionalPriceInCents === null ? null : formatMoney(variant.priceInCents), availableStock: variant.availableStock, isDefault: variant.isDefault }))} /></section>;
}
