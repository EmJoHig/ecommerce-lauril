...
export default async function CategoryPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const [{ slug }, query] = await Promise.all([params, searchParams]); const catalog = getCatalogService(); const [categories, settings] = await Promise.all([catalog.listCategories(), getPublicStoreSettings()]); const category = categories.find((item) => item.slug === slug); if (!category) notFound(); const search = text(query.buscar); const sort = text(query.orden); const page = await catalog.listProductPage({ categorySlug: category.slug, page: positive(query.pagina), pageSize: 12, search, sort: validSort(sort) }); return <PublicCatalog categories={categories} currentCategory={category.slug} heading={category.name} page={page} search={search} sort={sort} storeName={settings.storeName} />;
}
