...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const query = await searchParams; const category = text(query.categoria); const fragrance = text(query.fragancia); const search = text(query.buscar); const sort = text(query.orden);
  const catalog = getCatalogService();
  const [page, categories, fragrances, settings] = await Promise.all([catalog.listFilteredProductPage({ page: positive(query.pagina), pageSize: 12, search, sort: validSort(sort), fragrance, ...(category ? { categorySlug: category } : {}) }), catalog.listCategories(), catalog.listFragrances(), getPublicStoreSettings()]);
  return <PublicCatalog categories={categories} currentCategory={category || undefined} currentFragrance={fragrance || undefined} fragrances={fragrances} page={page} search={search} sort={sort} storeName={settings.storeName} />;
}
