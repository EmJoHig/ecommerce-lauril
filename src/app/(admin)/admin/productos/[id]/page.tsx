...
export default async function AdminProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <div className="admin-detail-grid"><section className="admin-panel"><div className="panel-heading"><div><p className="eyebrow">General</p><h2>Publicación</h2></div><span className={`status-badge status-badge--${product.status.toLowerCase()}`}>{product.status}</span></div><dl className="detail-list"><dt>Categorías</dt><dd>{categoryNames.join(" · ") || "Sin categoría"}</dd><dt>Destacado</dt><dd>{product.featured ? "Sí" : "No"}</dd><dt>Descripción corta</dt><dd>{product.shortDescription ?? "—"}</dd><dt>Descripción</dt><dd>{product.description ?? "—"}</dd></dl></section>
      <section className="admin-panel"><div className="panel-heading"><div><p className="eyebrow">Imágenes</p><h2>{product.images.length} referencias</h2></div></div>{product.images.length ? <div className="admin-image-grid">{product.images.map((image, index) => <figure key={image.id}><div><Image alt={image.altText} fill sizes="160px" src={image.url} unoptimized={isLocalUploadUrl(image.url)} /></div><figcaption>{index === 0 ? "Principal · " : ""}{image.altText}</figcaption></figure>)}</div> : <div className="empty-state empty-state--small"><p>Sin imágenes.</p></div>}</section></div>
