...
export default async function AdminProductsPage({ searchParams }: { searchParams: Promise<AdminSearchParams> }) {
  ...
  const [categories, page] = await Promise.all([
    ...
    service.listProducts({
      ...
      categoryId: textParam(parameters.categoria), sort: textParam(parameters.orden),
    }),
  ]);
  return <>
    <div className="admin-heading"><div><p className="eyebrow">Catálogo</p><h1>Productos</h1><p>{page.total} productos encontrados.</p></div><div className="form-actions"><Link className="button button--secondary" href="/admin/productos/importar">Importar Excel</Link><Link className="button button--dark" href="/admin/productos/nuevo">Nuevo producto</Link></div></div>
    ...
    <section className="admin-panel admin-table-wrap">{page.items.length === 0 ? <div className="empty-state empty-state--small"><h2>No hay resultados</h2><p>Probá modificando los filtros.</p></div> : <table className="admin-table admin-table--products"><thead><tr><th>Imagen</th><th>Producto</th><th>SKU</th><th>Categoría</th><th>Precio</th><th>Stock</th><th>Estado</th><th>Actualizado</th><th>Acciones</th></tr></thead><tbody>{page.items.map((product) => <tr key={product.id}><td><div className="admin-thumb"><Image alt={product.imageAlt ?? product.name} fill sizes="56px" src={product.imageUrl ?? "/product-placeholder.svg"} unoptimized={isLocalUploadUrl(product.imageUrl)} /></div></td><td><strong>{product.name}</strong><small>/{product.slug}</small></td><td>{product.primarySku}</td><td>{product.categoryNames.join(" · ") || "Sin categoría"}</td><td>{formatMoney(product.priceInCents)}</td><td>{product.availableStock}</td><td><span className={`status-badge status-badge--${product.status.toLowerCase()}`}>{product.status}</span></td><td>{product.updatedAt.toLocaleDateString("es-AR")}</td><td><div className="table-actions"><Link href={`/admin/productos/${product.id}`}>Detalle</Link><Link href={`/admin/productos/${product.id}/editar`}>Editar</Link>{product.status === "ACTIVE" ? <Link href={`/producto/${product.slug}`} target="_blank">Ver tienda</Link> : null}<form action={setProductStatusAction}><input name="id" type="hidden" value={product.id} /><input name="status" type="hidden" value={product.status === "ACTIVE" ? "INACTIVE" : "ACTIVE"} /><button type="submit">{product.status === "ACTIVE" ? "Desactivar" : "Activar"}</button></form></div></td></tr>)}</tbody></table>}</section>
    <nav className="pagination" aria-label="Paginación"><Link aria-disabled={page.page <= 1} href={adminPageHref("/admin/productos", parameters, page.page - 1)}>← Anterior</Link><span>Página {page.page} de {page.pageCount}</span><Link aria-disabled={page.page >= page.pageCount} href={adminPageHref("/admin/productos", parameters, page.page + 1)}>Siguiente →</Link></nav>
  </>;
}
