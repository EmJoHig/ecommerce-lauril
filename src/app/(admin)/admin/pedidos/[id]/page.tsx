...
export default async function AdminOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <div className="admin-grid">
      <section className="admin-panel form-section"><h2>Productos</h2>{order.items.map((item) => <div className="order-line" key={item.id}><span><strong>{item.productId ? <Link href={`/admin/productos/${item.productId}`}>{item.productName}</Link> : item.productName}</strong><small>{item.variantName} · SKU {item.sku}</small><small>{formatMoney(item.unitPriceInCents)} × {item.quantity} · <Link href={`/admin/stock/movimientos?buscar=${encodeURIComponent(item.sku)}`}>movimientos</Link></small></span><strong>{formatMoney(item.subtotalInCents)}</strong></div>)}</section>
