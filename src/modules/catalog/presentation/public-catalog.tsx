...
export function PublicCatalog({ page, categories, fragrances, currentCategory, currentFragrance, search, sort, storeName, heading = "Piezas para hacer hogar" }: Readonly<{
  ...
  const bannerProduct = page.items.find((product) => product.imageUrl && product.imageUrl !== "/product-placeholder.svg") ?? page.items[0];
      ...
      <div className="catalog-layout">
        <aside className="catalog-sidebar">
          <form className="public-filters" id="buscar">
            ...
            <label>Fragancia<select defaultValue={currentFragrance ?? ""} name="fragancia"><option value="">Todas las fragancias</option>{fragrances.map((fragrance) => <option key={fragrance.key} value={fragrance.key}>{fragrance.name}</option>)}</select></label>
